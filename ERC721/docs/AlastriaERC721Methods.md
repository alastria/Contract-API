...
## Get all tokens a wallet owns
Retrieves all the token IDs that a wallet owns.
### Request Endpoint
...
```json
{
    ...
    "result": [] // The result from the blockchain with all the token IDs that this wallet owns
}
```
### Error Example
```json
